...
namespace Client {
ClientApp::ClientApp() : state_{State::NONE} {}

void ClientApp::init(const std::vector<std::string>& argv,
...
void ClientApp::onTerminate() { tcpClient->disconnect(); }

void ClientApp::onJoin() {}
...
void ClientApp::setupSignals() {
  FW::G::Application->signals.onTerminate.connect(
      "ClientApp::onTerminate()", &ClientApp::onTerminate, this);
  FW::G::Application->signals.onJoin.connect("ClientApp::onJoin()",
                                             &ClientApp::onJoin, this);
  FW::G::Application->signals.onStartMainLoop.connect(
      "ClientApp::onJoin()",
      [=]() { FW::G::Logger.info(config.appName + " status: [Online]"); });
}

void ClientApp::setupRsa() {
...
void ClientApp::setupDatabase() {}

void ClientApp::setupTcpListeners() {
  tcpClient = std::make_shared<FW::Net::TcpClient>(
